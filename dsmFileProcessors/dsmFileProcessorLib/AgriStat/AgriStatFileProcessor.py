...
from dsmFileProcessorLib.AgriStat.process import process
...
class AgriStatFileProcessor():
    ...
    def onNewFile(self, inputBytes: io.BytesIO) -> Exception :
            ...
            with zipfile.ZipFile(inputBytes, 'r') as zip_ref:
                ...
                for file in filelist:
                    with zip_ref.open(file.filename, 'r') as zip:
                        ...
                        azureFunctions.uploadBytes(azUtil=azUtil, data=zip.read(), virtualFolderName=config.folderName, blobName=outFileName)
            return None
